/**
 * A decision-support extension of TranslationFamilyTree: the same twelve
 * translations, plus fourteen candidates the site is weighing (see
 * MEMORY.md / project_bible_guide_candidate_translations — not committed
 * to any of these yet): thirteen real translations, plus The Message as
 * the one paraphrase judged good enough to weigh alongside them (Peterson's
 * own Hebrew/Greek work + a 20-scholar review, unlike the site's other two
 * paraphrases — see this component's figcaption). Gold dashed boxes are
 * candidates; navy is unchanged from the real tree; pale boxes are
 * non-candidate ancestors kept only to show lineage.
 *
 * Layout (2026-09-16, owner sketch): hand-placed x, same convention as
 * TranslationFamilyTree — a literal cx per node, chosen so edges don't run
 ...
 *
 * The **y** axis is not hand-placed at all — it's a strict, shared
 * chronological rank across every node here (independents excluded; see
 * below): every unique year gets the next row down, so a node can never sit
 * lower than another node with a later year, full stop, regardless of which
 * branch or zone either one is in. (Owner correction, 2026-09-16: an
 * earlier pass hand-picked y per node for clean branch shapes and broke
 * this — e.g. NKJV 1982 level with YLT 1862, AMP 1965 level with NASB
 * 1971 — which reads as wrong the moment two nodes with different years
 * share a row.)
 *
 * The one deliberate exception to "real tree shape": a second, plain box
 * labeled just "ASV" (no year/code, so it can't be mistaken for a genuine
 * Majority Text translation) sits beside the WEB, joined to it by a red
 * connector — a cross-reference, not a real node, there only so the WEB's
 * own line doesn't have to cross the whole diagram to reach the real ASV.
 * It carries no line back to the real ASV itself; the shared label and
 * hover tooltip are what tie the two together.
 *
...
// derived below from a shared chronological rank so it can't drift back out
// of year order the way hand-picked y values did before.
const nodes: Node[] = [
  ...
  { id: "nirv", label: "NIrV", year: 1996, cx: 120, candidate: true, basis: "CT", phil: "Dynamic" },
  { id: "icb", label: "ICB", year: 1986, cx: 220, basis: "CT", phil: "Dynamic" },
  { id: "ncv", label: "NCV", year: 1991, cx: 220, candidate: true, basis: "CT", phil: "Dynamic" },
  ...
  { id: "bsb", label: "BSB", year: 2023, cx: 510, candidate: true, basis: "CT", phil: "Formal" },
  ...
  { id: "mev", label: "MEV", year: 2014, cx: 710, candidate: true, basis: "TR", phil: "Formal" },
  ...
  { id: "lsv", label: "LSV", year: 2020, cx: 900, w: 96, candidate: true, basis: "TR/MT", phil: "Formal" },
  { id: "web", label: "WEB", year: 2020, cx: 1020, candidate: true, basis: "MT", phil: "Formal" },
  { id: "msb", label: "MSB", year: 2023, cx: 1020, candidate: true, basis: "MT", phil: "Formal" },
];
...
// line almost invisible against its own boxes (owner: "so you can see the
// line").
...
// jog at the midpoint, into the child's top) — the same shape as
// TranslationFamilyTree, just with the drop point chosen per edge so nothing
// runs through an intervening box.
...
// The ASV stand-in: a plain (non-red) box, same style as the other
// non-candidate ancestors, at the real ASV's own row. Only its connector
// down to the WEB is red — a cross-reference, not a real fourth Majority
// Text translation, and (owner correction) it carries no separate line back
// to the real ASV — the shared label and tooltip do that work instead.
const ASV_STANDIN = { cx: 1020, y: yOf["asv"], w: 70 };

type Chip = { id: string; label: string; year: number; basis: TextualBasis; phil: Philosophy; current?: boolean };
const independents: Chip[] = [
  ...
  { id: "nlt", label: "NLT", year: 1996, basis: "CT", phil: "Dynamic", current: true },
  { id: "message", label: "The Message", year: 2002, basis: "CT", phil: "Paraphrase" },
  { id: "net", label: "NET", year: 2005, basis: "CT", phil: "Mediating", current: true },
  { id: "ceb", label: "CEB", year: 2011, basis: "CT", phil: "Dynamic", current: true },
  ...
  { id: "csb", label: "CSB", year: 2017, basis: "CT", phil: "Mediating", current: true },
...
function NodeBox({
  x, y, w, cx, current, candidate, label, year, basis, phil, title,
}: {
  x: number; y: number; w: number; cx: number; current?: boolean; candidate?: boolean;
  label: string; year: string; basis: TextualBasis; phil: Philosophy; title?: string;
}) {
  const labelColor = current ? "fill-white" : candidate ? "fill-brand-800" : "fill-neutral-700";
  const metaColor = current ? "fill-brand-200" : candidate ? "fill-gild-700" : "fill-neutral-400";
  return (
    <g>
      <rect
        ...
        className={
          current ? "fill-brand-800 stroke-brand-800" : candidate ? "fill-white stroke-gild-500" : "fill-white stroke-neutral-300"
        }
      />
...
export default function CandidateTranslationTree() {
  return (
    <figure className="mt-6">
            ...
            {independents.map((t) => (
              <span
                ...
                className={`inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs font-semibold ${
                  t.current ? "bg-brand-800 text-white" : "border border-dashed border-gild-500 bg-white text-brand-800"
                }`}
              >
                <span className="h-2 w-2 shrink-0 rounded-full ring-1 ring-black/10" style={{ background: PHIL_COLOR[t.phil] }} />
                {t.label}{" "}
                <span className={`font-normal ${t.current ? "text-brand-200" : "text-gild-700"}`}>
                  {t.year} {t.basis}
                </span>
              </span>
            ))}
            ...
            {nodes.map((n) => {
              ...
              return (
                <g key={n.id}>
                  ...
                  <NodeBox {...p} current={n.current} candidate={n.candidate} label={n.label} year={String(n.year)} basis={n.basis} phil={n.phil} title={FULL_NAME[n.id]} />
                </g>
              );
            })}
      ...
      <figcaption className="mt-4 max-w-2xl text-xs leading-relaxed text-neutral-500">
        ...
        <strong>LSB</strong>) still hang off their real KJV-line parents even where that reads as
        inside the Textus Receptus bracket. Top to bottom is a strict shared year axis: no box
        sits lower than another box with a later year, in any zone or branch &mdash; the whole
        reason NKJV (1982) now sits well below the ASV-descended translations that came out
        between it and the KJV, and AMP (1965) sits just above NASB (1971) rather than beside it.
        <strong> YLT</strong> leads to the <strong>LSV</strong>; the <strong>BSB</strong>&rsquo;s
        Majority Text sibling, the <strong>MSB</strong> &mdash; the same translation with its New
        Testament swapped to the Majority Text, from the same team on the same day in 2023 &mdash;
        sits on the right with a connector back to it. The red <strong>ASV</strong> box, at the
        real ASV&rsquo;s own row, is a cross-reference rather than a fourth Majority Text
        translation &mdash; it carries no year or code for that reason, and its one red line to
        the <strong>WEB</strong> (a modernization of the ASV&rsquo;s own wording) is what lets
        that connection skip crossing the whole diagram. The <strong>NIrV</strong> is the
        NIV&rsquo;s own text simplified to a third-grade reading level; the <strong>NCV</strong> a
        1991 revision of the International Children&rsquo;s Bible. To the left, the fully
        independent translations &mdash; no documented lineage at all &mdash; are their own list,
        sorted by year but not held to the tree&rsquo;s shared axis. Hover any abbreviation for its
        full name.
        <br />
        <br />
        ...
        It still earns a look: Eugene Peterson held a Master&rsquo;s in Semitic Languages and
        worked directly from the Hebrew and Greek himself, and twenty biblical scholars reviewed
        the result &mdash; a different starting point than the Living Bible&rsquo;s
        paraphrase-of-an-English-translation approach. (The site tracks two other paraphrases, the
        Living Bible and The Passion Translation, but neither is a candidate; see the spectrum
        below for where all three sit.)
      </figcaption>
    </figure>
  );
}
