...
export default function CountTable({ title, rows }: { title: string; rows: CountRow[] }) {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-neutral-200 bg-white">
      ...
      {/* table-fixed + explicit Count/% widths so those two columns are
          always visible — Name gets whatever's left and truncates with an
          ellipsis (full label on hover) instead of forcing the table wider
          than its container, which used to push % off the visible edge. */}
