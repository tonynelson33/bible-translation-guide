...
export default function CountTable({ title, rows }: { title: string; rows: CountRow[] }) {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-neutral-200 bg-white">
      <h2 className="border-b border-neutral-200 px-4 py-3 text-sm font-semibold text-neutral-800">
        {title}
      </h2>
          ...
          <tbody>
            {rows.map((row, i) => (
              <tr key={row.label} className={i % 2 === 0 ? "bg-white" : "bg-neutral-50/50"}>
                <td className="px-4 py-1.5 text-neutral-700">{row.label}</td>
                <td className="px-4 py-1.5 text-right tabular-nums text-neutral-700">
